...
namespace ArcCore.MonoBehaviours.EntityCreation
{
    public class ArcEntityCreator : MonoBehaviour
    {
        ...
        private void CreateHeightIndicator(AffArc arc, Material material)
        {
            ...
            float height = Conversions.GetWorldY(arc.startY) - 0.45f;

            float x = Conversions.GetWorldX(arc.startX); 
            float y = height / 2;
            const float z = 0;
        ...
        private void CreateHeadSegment(AffArc arc, Material material)
        {
            ...
            float x = Conversions.GetWorldX(arc.startX); 
            float y = Conversions.GetWorldY(arc.startY); 
            const float z = 0;
        ...
        private unsafe void CreateJudgeEntities(AffArc arc, int colorId, ArcFunnel* arcFunnelPtr, List<Entity> createdJudgeEntities)
        {
            ...
            while (timeF < arc.endTiming)
            {
                ...
                float arcStartX = Conversions.GetWorldX(arc.startX);
                float arcStartY = Conversions.GetWorldY(arc.startY);
                float arcEndX = Conversions.GetWorldX(arc.endX);
                float arcEndY = Conversions.GetWorldY(arc.endY);

                LinearPosGroup currentLpg = new LinearPosGroup()
                {
                    startPosition = new float2(
                        Conversions.GetXAt(math.unlerp(arc.timing, arc.endTiming, timeF), arcStartX, arcEndX, arc.easing),
                        Conversions.GetYAt(math.unlerp(arc.timing, arc.endTiming, timeF), arcStartY, arcEndY, arc.easing)
                    ),
                    startTime = (int)timeF,
                    endPosition = new float2(
                        Conversions.GetXAt(math.unlerp(arc.timing, arc.endTiming, timePosEnd), arcStartX, arcEndX, arc.easing),
                        Conversions.GetYAt(math.unlerp(arc.timing, arc.endTiming, timePosEnd), arcStartY, arcEndY, arc.easing)
                    ),
