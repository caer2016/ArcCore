...
namespace ArcCore.MonoBehaviours.EntityCreation
{
    ...
    public class TraceEntityCreator : MonoBehaviour
    {
        ...
        public void CreateEntities(List<AffTrace> affTraceList)
        {
            ...
            foreach (AffTrace trace in affTraceList)
            {
                ...
                float3 end = new float3(
                    Conversions.GetWorldX(trace.startX),
                    Conversions.GetWorldY(trace.startY),
                    (float)endfpos
                );
                ...
                for (int i=0; i<segmentCount - 1; i++)
                {
                    ...
                    endfpos = Conductor.Instance.GetFloorPositionFromTiming(trace.timing + t, trace.timingGroup);
                        ...
                        Conversions.GetWorldX(Conversions.GetXAt((float)t / duration, trace.startX, trace.endX, trace.easing)),
                        Conversions.GetWorldY(Conversions.GetYAt((float)t / duration, trace.startY, trace.endY, trace.easing)),
                        (float)endfpos
                    );
                ...
                endfpos = Conductor.Instance.GetFloorPositionFromTiming(trace.endTiming, trace.timingGroup);
                    ...
                    Conversions.GetWorldX(trace.endX),
                    Conversions.GetWorldY(trace.endY),
                    (float)endfpos
                );
        ...
        private void CreateHeadSegment(AffTrace trace)
        {
            ...
            float x = Conversions.GetWorldX(trace.startX); 
            float y = Conversions.GetWorldY(trace.startY); 
            const float z = 0;
